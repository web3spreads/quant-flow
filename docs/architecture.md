...
成交监控器，以及一条成交事件流（见下）。三者共用一把交易锁：`tryAcquire()` 非阻塞，冲突时后来者跳过本轮而不排队；
...
成交确认原本只在周期开头做一次：一笔开仓成交最长要等一整个 `interval_minutes` 才挂出
reduce_only 平仓单，而那段时间恰恰是价格刚穿过该层、最可能再穿回来的时候——止盈价被越过
又回落，这一轮的价差白丢，库存却留下了。`src/trading/fillStream.ts` 订阅本账户成交
（只需地址，不需私钥），去重后提示引擎提前同步一次。

三条约束让它不能变成风险源：

| 约束 | 做法 |
|---|---|
| 不是真值来源 | 事件只回答「可能有成交」，成交与否仍由 `userFills` + `getOpenOrders` 这条既有 REST 路径确认。掉线/丢消息/乱序只让节奏退回周期，不产生错误状态判定 |
| 永不新增敞口 | 走 `syncGridIncremental(symbol, allowOpen=false)`：只认领成交、挂 reduce_only 平仓单、结算 round-trip。否则一条 WebSocket 消息就能绕过形态闸门、趋势过滤与连亏锁定去开仓 |
| 不并发动账户 | 与网格周期共用交易锁，`tryAcquire` 拿不到就有界重试后让位——周期自己会做同一件事 |

回测不注入这条流，因此策略与簿记在回测与生产之间依然零分叉。重连后的历史快照由成交 id
去重挡掉，但**掉线期间漏掉的成交会出现在快照里并照常触发**；首次快照只播种去重集合。
...
缺口数、丢弃数与收包延迟分位数——延迟是研究阶段延迟模型的输入。

清单里两套口径不可互相替代：**`files.*` 描述文件**（字节、行数、sha256、有数据的秒数、
覆盖率），**`channels.*` 描述录制进程这一段**（消息数、缺口、丢弃、延迟）。覆盖率是研究的
样本纳入门槛，必须取前者——进程内计数器只覆盖本进程存活区间，当天重启过就低报，会把完好的
一天挤出样本（09-06 因此报成恰好卡在门槛上）。覆盖率在 gzip 校验那一遍顺带从文件算出，
不额外解压一遍；`book-verify --refresh` 可就地补算旧清单，且 sha256 不符时拒绝改写——
否则会用坏文件的摘要覆盖好摘要，把损坏永久藏起来。频道静默超 60 秒告警一次（预注册全部频道，连接死掉时不会一片安静）；重连以 error/close
