你将看到关于 {{ symbol }} 的最新交易记录、统计信息以及旧的经验规则。请基于这些输入输出 JSON 结果，以帮助后续交易复用经验或修正错误，重点识别以下问题：
- 在低波动/无量（成交量未达阈值、布林带变窄）下仍开仓
- 盈亏比 < 2.0 或 profit_to_fee_ratio < 3 仍进场
- 未遵守冷却（无新结构信号还重复交易）
- 单笔风险 > 1% 权益或金额 > 5% 余额
- do_nothing 未写明原因或未被触发但应当触发
- 同时提炼成功与失败交易的共性：成功时的趋势/量能/波动/价位结构/仓位与杠杆，失败时的噪声区、量能不足、盈亏比不足或过大仓位
