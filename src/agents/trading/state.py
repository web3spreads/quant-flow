...
class TradingAgentState(TypedDict):
    ...
    enriched_data: Optional[Dict[str, Any]]  # 增强数据（如外部信息）

    # ===== 持仓信息 =====
    current_positions: List[Dict[str, Any]]  # 当前持仓列表
    max_positions: int  # 最大持仓数
    balance_info: Optional[Dict[str, float]]  # 账户余额信息
    ...
    # ===== 历史上下文 =====
    historical_summary: Optional[str]  # 历史决策汇总

    # ===== 决策相关 =====
    prompt: str  # 生成的 Prompt
    decision_type: str  # 决策类型 (BUY, SELL, SELL_SHORT, BUY_TO_COVER, DO_NOTHING, BUY_SPOT_RECOMMEND)
