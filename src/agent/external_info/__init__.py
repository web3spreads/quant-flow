...
使用 Pydantic AI 实现的外部信息收集工作流。
负责从外部来源（Exa 搜索等）收集加密货币市场信息。

模块结构：
- state.py: 工作流状态定义 (ResearchState)
- tools.py: Exa 搜索工具（使用原生 HTTP 请求直接调用 Exa API）
- workflow.py: 原生 Python + Pydantic AI 工作流实现
"""
...
def __getattr__(name):
    """
    延迟导入：只在真正需要时才导入依赖 Exa HTTP / Pydantic AI 的模块
    """
    # 工具相关
    if name in (
