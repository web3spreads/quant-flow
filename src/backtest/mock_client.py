...
class MockHyperliquidClient:
    ...
    def place_market_order(
        ...
        return {
            ...
            'response': {
                'type': 'order',
                'data': {
                    'statuses': [{'resting': {'oid': f'mock_{len(self.trade_history)}'}}]
                }
            },
            'symbol': symbol,
            'is_buy': is_buy,
    ...
    def cancel_order(self, symbol: str, oid: int) -> Dict[str, Any]:
        ...
        self.open_orders = [
            ...
            if not (o.get('coin') == symbol and o.get('oid') == oid)
        ]
    ...
    def sell_spot(self, symbol: str, size: Optional[float] = None) -> Dict[str, Any]:
        """
        卖出现货（模拟，回测中暂不支持）
    ...
    def remove_position(self, symbol: str, position_id: Optional[int] = None):
        """
